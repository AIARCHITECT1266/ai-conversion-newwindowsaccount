...
function verifySignature(rawBody: string, signature: string, secret: string): boolean {
  // Paddle sendet: ts=TIMESTAMP;h1=HASH
...
// Transaktion abgeschlossen: Tenant anlegen
...
async function sendWelcomeEmail(
  ...
  try {
    await resend.emails.send({
      ...
      to: email,
      subject: `Willkommen bei AI Conversion – ${plan.charAt(0).toUpperCase() + plan.slice(1)} Plan`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a1a; color: #e2e8f0; padding: 40px; border-radius: 16px;">
          ...
          <p style="margin-top: 16px; line-height: 1.6;">
            ...
            Ihr <strong>${plan.charAt(0).toUpperCase() + plan.slice(1)} Plan</strong> ist jetzt aktiv.
    ...
    console.log("[Paddle] Willkommens-E-Mail gesendet an:", email);
  } catch (error) {
