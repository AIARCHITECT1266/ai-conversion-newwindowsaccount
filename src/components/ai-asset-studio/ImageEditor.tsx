...
// Standardwerte fuer alle Slider
const DEFAULTS = {
  sharpen: 0,
  brightness: 0,
  contrast: 0,
...
export default function ImageEditor() {
  ...
  // ---------- CSS-Filter fuer Live-Preview ----------
  const previewStyle = useMemo(() => {
    const filters: string[] = [];
    // Helligkeit: -100..+100 → CSS brightness 0.3..1.7
    if (brightness !== 0) filters.push(`brightness(${1 + (brightness / 100) * 0.7})`);
    // Kontrast: -100..+100 → CSS contrast 0.3..2.5
    if (contrast !== 0) {
      const c = contrast > 0 ? 1 + (contrast / 100) * 1.5 : 1 + (contrast / 100) * 0.7;
      filters.push(`contrast(${c})`);
    }
    // Saettigung: -100..+100 → CSS saturate 0..2
  ...
  }, [brightness, contrast, saturation, roundCorners]);

  // Pruefen ob Aenderungen gemacht wurden
  ...
  function resetSliders() {
    ...
    setVignette(DEFAULTS.vignette);
    setRoundCorners(DEFAULTS.roundCorners);
    setBrandKit("");
  }
  ...
  function Slider({ label, value, onChange, min = 0, max = 100, unit = "%", center }: {
    ...
    const pct = center
      ? Math.abs(value) / Math.max(Math.abs(min), max) * 100
      : (value / max) * 100;
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <span className={`text-xs tabular-nums ${value !== 0 ? "text-[#c9a84c]" : "text-[#ede8df]/30"}`}>
        ...
        <input
          ...
          className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-[#c9a84c] bg-[#ede8df]/10"
        />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {!preview ? (
        ...
        /* Live-Vorschau mit CSS-Filtern */
        <div className="relative rounded-xl border border-[#c9a84c]/10 overflow-hidden bg-[#0e0e1a]">
          {/* Vignette-Overlay (CSS, nicht im Bild) */}
          {vignette > 0 && (
            <div
              className="absolute inset-0 pointer-events-none z-10"
              style={{
                background: `radial-gradient(ellipse at center, transparent ${Math.max(10, 50 - (vignette / 100) * 30)}%, rgba(0,0,0,${0.3 + (vignette / 100) * 0.55}) 100%)`,
              }}
            />
          )}
          ...
          <img
            src={preview}
            alt="Vorschau"
            className="w-full max-h-80 object-contain transition-all duration-150"
            style={previewStyle}
          />
          <button
            type="button"
            onClick={clearFile}
            className="absolute top-2 right-2 z-20 rounded-full bg-black/60 p-1.5 text-white/70 hover:text-white transition-colors"
          >
            <X className="h-4 w-4" />
          </button>
      ...
      {preview && (
        <div className="space-y-4 rounded-xl border border-[#c9a84c]/10 bg-[#0e0e1a] p-4">
          <div className="flex items-center justify-between">
            ...
            {hasChanges && (
              <button
                ...
                className="flex items-center gap-1 text-xs text-[#ede8df]/30 hover:text-[#c9a84c] transition-colors"
              >
          ...
          <div className="grid gap-3">
            ...
            <Slider label="Schaerfe" value={sharpen} onChange={setSharpen} />
            <Slider label="Vignette" value={vignette} onChange={setVignette} />
            <Slider label="Abgerundete Ecken" value={roundCorners} onChange={setRoundCorners} unit="px" />
          </div>
      ...
      {result && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#ede8df]/50 uppercase tracking-wider">
              Ergebnis
            </span>
            <button
              type="button"
              onClick={handleDownload}
              className="flex items-center gap-1.5 rounded-lg border border-[#c9a84c]/20 bg-[#c9a84c]/10 px-3 py-1.5 text-xs font-medium text-[#c9a84c] transition-all hover:bg-[#c9a84c]/20"
            >
              <Download className="h-3.5 w-3.5" />
              Herunterladen (.{outputFormat})
            </button>
          </div>
